 ...
 *
 * @details
 * Alloue de la mémoire permettant de stocker @p num_objects, chacun de type
 * @p *p.
 *
 ...
 *
 * @attention
 * Le pointeur @p p doit avoir été déclaré, mais ne pas pointer vers une zone
 * déjà allouée dynamiquement (sous peine de créer une fuite mémoire).
 * Si @p p pointe déjà vers une zone allouée dynamiquement, il faut utiliser
 * REALLOC.
 *
 * @see REALLOC()
 */
 ...
 *
 * @details
 * Alloue de la mémoire initialisée à 0, permettant de stocker @p num_objects,
 * chacun de type @p *p.
 *
 * **Exemple d'utilisation :**
 * ```
 ...
 *
 * Ré-alloue la mémoire pointée par un pointeur @p p, dont la valeur doit avoir
 * été retournée précédemment par une fonction d'allocation dynamique (malloc,
 ...
 *                 // précédent en début ce u tableau, et affecte à p
 ...
 *
 * @attention
 * Le pointeur @p p doit avoir été déclaré et pointer vers une zone
 ...
 * ```
 * Ici, 3 est le nombre de pointeurs à vérifier, et @p p1, @p p2 et @p p3 sont
 les pointeurs. Si @p p1 et @p p3 sont NULL, la macro affichera un message
 ...
 * Dans l'exemple précédent, si @p p2 est NULL, on ne libère pas @p p3 et cela
